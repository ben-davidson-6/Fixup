...
class InferenceBuilder:
    def __init__(self, model_name, run, network):
        ...
        if run is not None:
            ...
            self.ckpt_path = tf.train.latest_checkpoint(model_dir)
        self.optimized_pb_path = os.path.join(model_dir, 'optimised_model.pb')
        self.network = network
        self.graph = tf.Graph()
        self.sess = tf.Session(graph=self.graph)

    def restore_graph(self, inputs, preprocess, variable_map, **kwargs):
        placeholder_inputs = {}
        with self.graph.as_default():
            ...
            if variable_map is not None:
                with tf.variable_scope('', reuse=True) as s:
                    var_list = {orig_name: tf.get_variable(variable_map[orig_name]) for orig_name in variable_map}
            else:
                var_list = None
            saver = tf.train.Saver(var_list=var_list, reshape=True)
            saver.restore(self.sess, self.ckpt_path)

    def to_optmised_graph(self, outputs, inputs, preprocess=None, variable_map=None, **kwargs):
        if type(outputs) != list:
            outputs = [outputs]
        outputs = [x[:-2] if (':0' == x[-2:]) else x for x in outputs]
        self.restore_graph(inputs, preprocess, variable_map, **kwargs)

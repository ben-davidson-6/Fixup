...
class TrainingModel:
    def __init__(self, network, dataset, debug=False):
    ...
    def add_training_op(self, training_op_func, lr_schedule, init_learn):
        """Hack for more complicated optimisation"""
        with self.graph.as_default():
            learning_rate = self._build_learning_rate(init_learn, lr_schedule)
            training_op_func(learning_rate)
    ...
    def restore_variables(self, restore_path, exclude_vars):
        print('getting variables can restore')

...
def margin_loss(labels, embedding, beta, params):
    ...
    poss_contribute_pairs = tf.count_nonzero(poss_loss, dtype=tf.float32)
    ...
    neg_contribute_pairs = tf.count_nonzero(neg_loss, dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    print(np.linalg.norm(embeddings[:, None] - embeddings[None], axis=2))

    labels = np.array([0, 1, 2])
    embeddings = tf.constant(embeddings.astype(np.float32))
    labels = tf.constant(labels)
    margins = tf.constant([2, 2, 2], dtype=tf.float32)

    log_weights, positive_mask, pairwise_distances = calc_weights(embeddings, labels, cutoff=0.5, add_summary=False)
    log_weights = zero_non_contributing_examples(log_weights, pairwise_distances, positive_mask, margins)
    b = tf.random.categorical(
        log_weights,
        10)
    ...
    with tf.Session() as sess:
        print(sess.run(b))
        print(sess.run(log_weights))
        print(sess.run(pairwise_distances))
        print(sess.run(pairwise_distances))
